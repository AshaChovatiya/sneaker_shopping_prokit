...
class SSSearchFragment extends StatelessWidget {
                              ...
                              : _searchScreenProvider
                                  ...
                                  ? ListView.builder(
                                      controller: _scrollController
                                        ..addListener(() async {
                                          await fetchNewData(
                                              context, _searchScreenProvider);
                                        }),
                                      padding:
                                          EdgeInsets.only(top: 0, bottom: 8),
                                      itemCount: _searchScreenProvider
                                          .searchProductItems?.length,
                                      // shrinkWrap: true,
                                      // physics: NeverScrollableScrollPhysics(),
                                      itemBuilder: (_, index) {
                                        if (_searchScreenProvider
                                                .isPagination &&
                                            index ==
                                                _searchScreenProvider
                                                        .searchProductItems!
  ...
  Future<void> fetchNewData(
      BuildContext context, SearchScreenProvider searchProvider) async {
    if (_scrollController.position.pixels ==
        _scrollController.position.maxScrollExtent) {
      await searchProvider.searchProductData(
          search: searchController.text, isScroll: true);
    }
  }
}
